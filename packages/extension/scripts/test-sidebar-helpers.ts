...
// new Date("not-a-date") yields an Invalid Date without throwing, and
// toLocaleDateString would render the literal string "Invalid Date". formatDate
// guards on Number.isNaN(d.getTime()) and returns "" instead, so unparseable
// input renders as nothing rather than leaking "Invalid Date" into the UI.
check(
  "invalid date string → empty string (not 'Invalid Date')",
